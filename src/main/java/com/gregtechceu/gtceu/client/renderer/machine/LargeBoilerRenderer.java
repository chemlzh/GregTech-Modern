        ...
        if (machine.self().getPos().below().getY() == part.self().getPos().getY()) {
            ...
            if (side != null && modelFacing != null) {
                if (side == Direction.UP) {
                    quads.add(StaticFaceBakery.bakeFace(modelFacing, ModelFactory.getBlockSprite(firebox.top()), modelState));
                } else if (side == Direction.DOWN) {
                    quads.add(StaticFaceBakery.bakeFace(modelFacing, ModelFactory.getBlockSprite(firebox.bottom()), modelState));
                } else {
                    quads.add(StaticFaceBakery.bakeFace(modelFacing, ModelFactory.getBlockSprite(firebox.side()), modelState));
                    if (machine instanceof IRecipeLogicMachine recipeLogicMachine && recipeLogicMachine.getRecipeLogic().isWorking()) {
                        quads.add(StaticFaceBakery.bakeFace(modelFacing, ModelFactory.getBlockSprite(BLOOM_OVERLAY), modelState, -101, 15, true, false));
                    }
                }
            }
        } else {
